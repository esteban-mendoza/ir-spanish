#!/usr/bin/env python3
"""
Rerank a first-stage retrieval run using jinaai/jina-colbert-v2
(late interaction retriever) and evaluate against MessIRve qrels.

jina-colbert-v2 is a ColBERT-style late interaction model: it encodes
queries and documents separately into multi-vector representations
(one embedding per token), then scores via MaxSim — the sum over query
tokens of the maximum cosine similarity with any document token.

Multi-GPU parallelism uses separate OS processes (spawn) to match the
proven-stable pattern from jina_reranker_v3.py.

Usage:
    python -m rerankers.jina_colbert_v2
"""
...
MAX_QUERY_LENGTH = 32       # ColBERT default (queries padded with [MASK] tokens)
MAX_DOC_LENGTH = 300        # ColBERT default per paper evaluation setup
DOC_BATCH_SIZE = 32         # documents per encode() batch
...
def _worker(
    ...
    """Rerank a shard of queries on one GPU using ColBERT late interaction.
    ...
    with torch.no_grad():
        for idx, (query_id, query_text, doc_ids) in enumerate(pending, 1):
            ...
            # Encode query and documents into multi-vector representations
...
def rerank(
    ...
    """Score documents with jina-colbert-v2 across multiple GPUs using
